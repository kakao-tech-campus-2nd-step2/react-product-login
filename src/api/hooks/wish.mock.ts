...
import { DeleteWishPath } from './useDeleteWish';
import { getWishesPath } from './useGetWish';
import { PostWishPath } from './usePostWish';
...
export const wishesMockHandlers = [
  ...
  rest.delete(DeleteWishPath(':wishId'), (req, res, ctx) => {
    const { wishId } = req.params as { wishId: string };
    console.log(`Wish ID to delete: ${wishId}`);
