...
Test code를 통해서 실제 인터랙션이나 컴포넌트 내에 mock data를 주입하지 않고도 컴포넌트의 올바른 동작 여부에 대해 확인할 수 있다는 점이 좋았다.
아쉬웠던 점으로는 아직까지 모듈 함수 목킹과 같은 (본인 기준) 심화적인 부분들을 잘 다루지 못한다는 점이었는데, 그나마 마지막에 어찌저찌 방법을 찾아내서
원하는 방향으로 테스트를 잘 진행했던 것 같다.

- 질문 2. 스스로 생각했을 때 좋은 컴포넌트란 무엇인지 본인만의 기준을 세우고 설명해 주세요.

좋은 컴포넌트란 사용자 입장에서는 직관적인 인터렉션과 간편한 UI를 갖는 컴포넌트라고 생각한다.
서비스와 데이터의 성격에 맞는 컴포넌트 설계를 잘 하는것이 매우 중요하지만 이것만큼 중요하게 여겨지는 것이
인터렉션을 통한 사용자 경험의 향상이라고 생각하기 때문이다.

개발자 입장에서는 코드 가독성과 재사용성의 보상점에 가까운 컴포넌트가 좋은 컴포넌트라고 생각한다. 기본적으로 컴포넌트의 재사용성이 높다면
중복되는 부분을 줄이면서 효율적인 앱 설계가 가능하겠지만 재사용성을 지나치게 높이면 코드 가독성을 해칠 수 있기 때문이다(ex. 타입 파라미터의 남용).

그리고 당연히 UI나 데이터의 흐름과 같은 주요 흐름들을 잘 파악할 수 있도록 적절한 코드 분리가 필요하다고 생각한다.

- 질문 3. 스스로 생각했을 때 공통 컴포넌트를 만들 때 가장 중요한 요소 2개를 선택하고 이유와 함께 설명해주세요.

1. 가독성

결국은 개발 자체가 아니라 유지 보수에 초점을 둬야 하는 것 같다. UI와 비즈니스 로직을 잘 분리하여 가독성 높은 코드를 만들어야
상대적 코드 수명이 짧은 프론트엔드 생태계에서 그나마 코드 수명을 길게 가져갈 수 있다.

2. 재사용성

재사용성이 높은 컴포넌트는 동일한 컴포넌트를 여러 번 작성하는 수고를 덜어주고, 하나의 동작을 여러 컴포넌트에서 각각 작동할 수 있게 하기 때문에
변경 사항이 생길 경우 재사용되는 컴포넌트만을 수정하면 모든 컴포넌트에 반영되므로 유지 보수가 간편해지기 때문이다.
